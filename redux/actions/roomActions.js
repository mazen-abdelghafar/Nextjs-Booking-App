...
export const getRoomReviews = (roomId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`/api/admin/reviews?roomId=${roomId}`);
    dispatch({
...
// delete room review action
export const deleteReview = (roomId, reviewId) => async (dispatch) => {
  try {
    dispatch({ type: DELETE_REVIEW_REQUEST });

    const { data } = await axios.put(
      `/api/admin/reviews?roomId=${roomId}&reviewId=${reviewId}`
    );
    dispatch({
